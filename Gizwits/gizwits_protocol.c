...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagRemind_drink_switch)
    {
        info->event[info->num] = EVENT_Remind_drink_switch;
        info->num++;
        dataPoints->valueRemind_drink_switch = gizStandardDecompressionValue(Remind_drink_switch_BYTEOFFSET,Remind_drink_switch_BITOFFSET,Remind_drink_switch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTotal_amount_of_water)
    {
        info->event[info->num] = EVENT_Total_amount_of_water;
        info->num++;
        dataPoints->valueTotal_amount_of_water = gizX2Y(Total_amount_of_water_RATIO,  Total_amount_of_water_ADDITION, exchangeBytes(issuedData->attrVals.valueTotal_amount_of_water));
    }
        
    if(0x01 == issuedData->attrFlags.flagRecommended_amount_of_water)
    {
        info->event[info->num] = EVENT_Recommended_amount_of_water;
        info->num++;
        dataPoints->valueRecommended_amount_of_water = gizX2Y(Recommended_amount_of_water_RATIO,  Recommended_amount_of_water_ADDITION, exchangeBytes(issuedData->attrVals.valueRecommended_amount_of_water));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueRemind_drink_switch != cur->valueRemind_drink_switch)
    {
        GIZWITS_LOG("valueRemind_drink_switch Changed\n");
        ret = 1;
    }
    if(last->valueTotal_amount_of_water != cur->valueTotal_amount_of_water)
    {
        GIZWITS_LOG("valueTotal_amount_of_water Changed\n");
        ret = 1;
    }
    if(last->valueRecommended_amount_of_water != cur->valueRecommended_amount_of_water)
    {
        GIZWITS_LOG("valueRecommended_amount_of_water Changed\n");
        ret = 1;
    }
    ...
    if(last->valueRSSI_1 != cur->valueRSSI_1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRSSI_1 Changed\n");
            ret = 1;
        }
    }
    if(last->valueRSSI_2 != cur->valueRSSI_2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRSSI_2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueRSSI_3 != cur->valueRSSI_3)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRSSI_3 Changed\n");
            ret = 1;
        }
    }
    if(last->valueMonth != cur->valueMonth)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMonth Changed\n");
            ret = 1;
        }
    }
    if(last->valueDay != cur->valueDay)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDay Changed\n");
            ret = 1;
        }
    }
    if(last->valueHour != cur->valueHour)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHour Changed\n");
            ret = 1;
        }
    }
    if(last->valueMinute != cur->valueMinute)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMinute Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valueLAC_1 != cur->valueLAC_1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLAC_1 Changed\n");
            ret = 1;
        }
    }
    if(last->valueCellID_1 != cur->valueCellID_1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCellID_1 Changed\n");
            ret = 1;
        }
    }
    if(last->valueLAC_2 != cur->valueLAC_2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLAC_2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueCellID_2 != cur->valueCellID_2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCellID_2 Changed\n");
            ret = 1;
        }
    }
    if(last->valueLAC_3 != cur->valueLAC_3)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLAC_3 Changed\n");
            ret = 1;
        }
    }
    if(last->valueCellID_3 != cur->valueCellID_3)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCellID_3 Changed\n");
            ret = 1;
        }
    }
    if(last->valueYear != cur->valueYear)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueYear Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Remind_drink_switch_BYTEOFFSET,Remind_drink_switch_BITOFFSET,Remind_drink_switch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRemind_drink_switch);
    ...
    devStatusPtr->valueRSSI_1 = gizY2X(RSSI_1_RATIO,  RSSI_1_ADDITION, dataPoints->valueRSSI_1); 
    devStatusPtr->valueRSSI_2 = gizY2X(RSSI_2_RATIO,  RSSI_2_ADDITION, dataPoints->valueRSSI_2); 
    devStatusPtr->valueRSSI_3 = gizY2X(RSSI_3_RATIO,  RSSI_3_ADDITION, dataPoints->valueRSSI_3); 
    devStatusPtr->valueMonth = gizY2X(Month_RATIO,  Month_ADDITION, dataPoints->valueMonth); 
    devStatusPtr->valueDay = gizY2X(Day_RATIO,  Day_ADDITION, dataPoints->valueDay); 
    devStatusPtr->valueHour = gizY2X(Hour_RATIO,  Hour_ADDITION, dataPoints->valueHour); 
    devStatusPtr->valueMinute = gizY2X(Minute_RATIO,  Minute_ADDITION, dataPoints->valueMinute); 

    devStatusPtr->valueTotal_amount_of_water = exchangeBytes(gizY2X(Total_amount_of_water_RATIO,  Total_amount_of_water_ADDITION, dataPoints->valueTotal_amount_of_water)); 
    devStatusPtr->valueRecommended_amount_of_water = exchangeBytes(gizY2X(Recommended_amount_of_water_RATIO,  Recommended_amount_of_water_ADDITION, dataPoints->valueRecommended_amount_of_water)); 
    devStatusPtr->valueWater = exchangeBytes(gizY2X(Water_RATIO,  Water_ADDITION, dataPoints->valueWater)); 
    devStatusPtr->valueLAC_1 = exchangeBytes(gizY2X(LAC_1_RATIO,  LAC_1_ADDITION, dataPoints->valueLAC_1)); 
    devStatusPtr->valueCellID_1 = exchangeBytes(gizY2X(CellID_1_RATIO,  CellID_1_ADDITION, dataPoints->valueCellID_1)); 
    devStatusPtr->valueLAC_2 = exchangeBytes(gizY2X(LAC_2_RATIO,  LAC_2_ADDITION, dataPoints->valueLAC_2)); 
    devStatusPtr->valueCellID_2 = exchangeBytes(gizY2X(CellID_2_RATIO,  CellID_2_ADDITION, dataPoints->valueCellID_2)); 
    devStatusPtr->valueLAC_3 = exchangeBytes(gizY2X(LAC_3_RATIO,  LAC_3_ADDITION, dataPoints->valueLAC_3)); 
    devStatusPtr->valueCellID_3 = exchangeBytes(gizY2X(CellID_3_RATIO,  CellID_3_ADDITION, dataPoints->valueCellID_3)); 
    devStatusPtr->valueYear = exchangeBytes(gizY2X(Year_RATIO,  Year_ADDITION, dataPoints->valueYear)); 
...
static void gizDevReportPolicy(dataPoint_t *currentData)
{
    ...
    if((0 == (timeNow % (2400000))) && (lastRepTime != timeNow))
    {
